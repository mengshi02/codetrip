...
func Test_layerNode_search(t *testing.T) {
	...
	best := entry.search(2, 4, nil, []float32{4}, func(a, b *layerNode[int]) float32 {
		return EuclideanDistance(a.Value, b.Value)
	})
...
func TestGraph_RemoveAllNodes(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		g := NewGraph[int]()
		if err := g.Add(MakeNode(1, vec)); err != nil {
			t.Fatalf("add: %v", err)
		}
		g.Delete(1)
		if err := g.Add(MakeNode(1, vec)); err != nil {
