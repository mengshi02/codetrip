// Package golang — Go binding merge precedence.
// Implements per-scope binding merge for Go: local bindings take precedence
...
// is dropped.
// Ported from TS languages/go/merge-bindings.ts.
package golang
...
// originTier maps BindingOrigin to its merge precedence tier.
...
// GoMergeBindings merges two binding sets for a Go scope, keeping the
// highest-precedence (lowest-tier) binding for each name.
// existing bindings are preferred over incoming when tiers are equal.
//
// Mirrors TS goMergeBindings(existing, incoming, scopeID).
// TODO: full implementation — currently concatenates and deduplicates by tier.
func GoMergeBindings(existing []shared.BindingRef, incoming []shared.BindingRef, scopeID string) []shared.BindingRef {
	if len(existing) == 0 {
		return incoming
	}
	if len(incoming) == 0 {
	...
	sort.SliceStable(all, func(i, j int) bool {
		return originTier(all[i].Origin) < originTier(all[j].Origin)
	})
