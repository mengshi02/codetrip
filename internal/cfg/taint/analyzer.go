...
// taintState represents taint state, uses sync.Pool for reuse
type taintState struct {
	...
	taintPaths map[string][]pipeline.HopInfo
...
var taintStatePool = sync.Pool{
	New: func() any {
		return &taintState{
			taintedVars:   make(map[string]bool),
			taintSources:  make(map[string]sourceInfo),
			taintPaths:    make(map[string][]pipeline.HopInfo),
			sanitizedVars: make(map[string]bool),
		}
	},
}

func getTaintState() *taintState {
	return taintStatePool.Get().(*taintState)
}

func putTaintState(s *taintState) {
...
func (a *DefaultTaintAnalyzer) Analyze(fcfg *cfg.FunctionCFG, registry *TaintModelRegistry) ([]TaintResult, error) {
	...
	for _, src := range sources {
		...
		state.taintPaths[src.varName] = []pipeline.HopInfo{
		...
		for line := block.StartLine; line <= block.EndLine; line++ {
			...
			if spec, ok := sanitizerLines[line]; ok {
				...
				for v := range state.taintedVars {
					...
					state.taintPaths[v] = append(state.taintPaths[v], pipeline.HopInfo{
						...
						for _, usedVar := range stmt.Uses {
							if state.taintedVars[usedVar] {
								...
								if path, ok := state.taintPaths[usedVar]; ok {
									newPath := make([]pipeline.HopInfo, len(path))
									copy(newPath, path)
									newPath = append(newPath, pipeline.HopInfo{
	...
	for _, sink := range sinks {
		for _, usedVar := range []string{sink.varName} {
			if state.taintedVars[usedVar] {
				...
				// Add sink to path
				hopPath := make([]pipeline.HopInfo, len(path))
				copy(hopPath, path)
				hopPath = append(hopPath, pipeline.HopInfo{
