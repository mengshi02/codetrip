...
type VectorSearch struct {
	...
	vecFile        *vecfile.VectorFileReader // mmap'd quantized vector file (nil if not loaded)
...
func (s *VectorSearch) LoadVectorFile() error {
	...
	repo := s.graph.Repo()
	path := vecfile.VectorFilePath(s.dataDir, repo)
	if !vecfile.VectorFileExists(s.dataDir, repo) {
		return nil // no vector file yet
	}
	vf, err := vecfile.OpenVectorFile(path)
	if err != nil {
...
// float32 slice pool, used for cosine calculation
var vecPool = sync.Pool{
	New: func() interface{} {
		s := make([]float32, 0, 384)
		return &s
	},
}
...
// BuildDualModalHNSWIndex builds separate HNSW indices for description and code modalities.
...
func (s *VectorSearch) BuildDualModalHNSWIndex() error {
	if s.embedder == nil || s.embedder.Dimensions() == 0 {
		return nil
	}
	...
	if s.dataDir != "" {
		if descIdx != nil {
			s.persistDualHNSWIndex(descIdx, repo, "desc")
		}
		if codeIdx != nil {
			s.persistDualHNSWIndex(codeIdx, repo, "code")
		}
	}
...
// dualHnswIndexPath returns the file path for a dual-modal persisted HNSW index.
// modality is "desc" or "code".
func (s *VectorSearch) dualHnswIndexPath(repo, modality string) string {
	return filepath.Join(s.dataDir, "index", repo, fmt.Sprintf("hnsw_%s.graph", modality))
}

// persistDualHNSWIndex saves a dual-modal HNSW index to disk.
func (s *VectorSearch) persistDualHNSWIndex(idx *hnsw.Graph[string], repo, modality string) {
	path := s.dualHnswIndexPath(repo, modality)
...
// RestoreDualHNSWIndex attempts to restore dual-modal HNSW indices from disk.
// Returns true if both desc and code indices were successfully restored.
func (s *VectorSearch) RestoreDualHNSWIndex() bool {
	if s.dataDir == "" {
		return false
	}

	repo := s.graph.Repo()
	descRestored := s.restoreModalityHNSW(repo, "desc")
	codeRestored := s.restoreModalityHNSW(repo, "code")
...
// restoreModalityHNSW restores a single-modality HNSW index from disk.
func (s *VectorSearch) restoreModalityHNSW(repo, modality string) bool {
	path := s.dualHnswIndexPath(repo, modality)
