...
// TestBuildDualModalHNSWIndex verifies that dual-modal HNSW indices
// can be built from stored embedding data.
func TestBuildDualModalHNSWIndex(t *testing.T) {
	s, gs, _ := setupDualModalData(t)
	defer s.Close()

	vs := NewVectorSearch(&mockEmbedder{dim: 8}, s, gs)
	...
	if !built {
		t.Error("dualHnswBuilt should be true after BuildDualModalHNSWIndex")
	}
	if descIdx == nil {
...
// TestBuildDualModalHNSWIndex_NoData verifies that BuildDualModalHNSWIndex
// handles the case where no dual-modal embedding data exists.
func TestBuildDualModalHNSWIndex_NoData(t *testing.T) {
	dir := t.TempDir()
	s, err := store.Open(store.Config{Path: dir})
	if err != nil {
		t.Fatalf("open store: %v", err)
...
// TestRestoreDualHNSWIndex verifies that dual-modal HNSW indices
// can be persisted and restored.
func TestRestoreDualHNSWIndex(t *testing.T) {
	dir := t.TempDir()
	s, err := store.Open(store.Config{Path: dir})
	if err != nil {
		t.Fatalf("open store: %v", err)
	...
	vs := NewVectorSearchWithDir(&mockEmbedder{dim: 8}, s, gs, dir)
	...
	vs2 := NewVectorSearchWithDir(&mockEmbedder{dim: 8}, s, gs, dir)
	restored := vs2.RestoreDualHNSWIndex()
	if !restored {
		t.Error("RestoreDualHNSWIndex should return true when persisted indices exist")
	}
...
// BenchmarkBuildDualModalHNSWIndex benchmarks building dual-modal HNSW indices.
func BenchmarkBuildDualModalHNSWIndex(b *testing.B) {
	dir := b.TempDir()
	s, err := store.Open(store.Config{Path: dir})
	if err != nil {
...
func BenchmarkSearchDualModal(b *testing.B) {
	...
	vs := NewVectorSearch(&mockEmbedder{dim: dim}, s, gs)
	if err := vs.BuildDualModalHNSWIndex(); err != nil {
