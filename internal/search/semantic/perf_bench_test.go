...
func buildHNSWIndexQuantized(b *testing.B, vs *VectorSearch, gs *graph.GraphStore, s *store.Store, dataDir string, nodeCount int) {
	...
	vfWriter := NewVectorFileWriter(dim, scale, offset)
	for i := 0; i < nodeCount; i++ {
...
// BenchmarkPerf_BuildSemanticIndex: measures dual-modal HNSW index build time.
// Target: BuildSemanticIndex构建时间 < 15分钟
func BenchmarkPerf_BuildSemanticIndex(b *testing.B) {
	for i := 0; i < b.N; i++ {
		b.StopTimer()
		vs, gs, s, _, cleanup := openPerfVectorSearch(b)
...
func BenchmarkPerf_BM25Search_Large(b *testing.B) {
	...
	idx, err := symbol.NewLexicalIndexWithDir(dir, "perfrepo", s)
	if err != nil {
		b.Fatal(err)
	}
	defer idx.Close()
...
func BenchmarkPerf_BM25BatchIndex_10K(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		cfg.CacheSize = 64 << 20
		s, _ := store.Open(cfg)
		idx, _ := symbol.NewLexicalIndexWithDir(dir, "perfrepo", s)
