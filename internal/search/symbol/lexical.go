...
// LexicalIndex is a Bluge-based BM25 full-text index
...
// NewLexicalIndexWithDir creates a persistent BM25 index using two-phase build.
...
func NewLexicalIndexWithDir(dataDir string, repo string, store *store.Store) (*LexicalIndex, error) {
	idx := &LexicalIndex{
		repo:    repo,
		dataDir: dataDir,
		store:   store,
...
func (idx *LexicalIndex) buildTwoPhase(dataDir, repo, buildDir, finalDir string) (*LexicalIndex, error) {
...
func (idx *LexicalIndex) FinalizeBuild() error {
	if idx.dataDir == "" {
...
// LexicalResult represents a BM25 search result
...
func (idx *LexicalIndex) IndexNode(node *graph.Node) error {
	if idx.blugeWriter == nil {
...
func (idx *LexicalIndex) BatchIndex(nodes []*graph.Node) error {
	if idx.blugeWriter == nil {
		return fmt.Errorf("bluge writer not initialized")
	}
	if len(nodes) == 0 {
...
func (idx *LexicalIndex) BatchIndexChunked(nodes []*graph.Node, chunkSize int) error {
	if idx.blugeWriter == nil {
		return fmt.Errorf("bluge writer not initialized")
	}
	if len(nodes) == 0 {
...
func (idx *LexicalIndex) Search(queryStr string, limit int) ([]LexicalResult, error) {
	if idx.blugeWriter == nil {
		return nil, nil
	}
...
func (idx *LexicalIndex) Repo() string {
	return idx.repo
}

// DocumentCount returns the number of documents in the index
func (idx *LexicalIndex) DocumentCount() (uint64, error) {
	if idx.blugeWriter == nil {
		return 0, nil
	}
...
func (idx *LexicalIndex) documentToBluge(doc *SearchDocument) *bluge.Document {
