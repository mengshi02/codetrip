...
type PipelineResult struct {
	...
	Skipped       int // incrementally skipped
	Errors        int
}
...
// Run executes the dual-modal embedding pipeline with incremental hash checking.
func (p *EmbeddingPipeline) Run(ctx context.Context, repo string) (*PipelineResult, error) {
	return p.RunDualModal(ctx, repo, true)
}
...
func (p *EmbeddingPipeline) RunDualModal(ctx context.Context, repo string, incremental bool) (*PipelineResult, error) {
	...
	for iter.Next() {
		...
		// Incremental embedding: SHA1 content hash comparison
		if incremental {
			hashKey := graph.EmbHashKey(repo, node.ID)
			existingHash, err := p.store.Get([]byte(hashKey))
			if err == nil {
				// Hash both description and code content for comprehensive comparison
				descText := buildDescriptionText(node, ec)
				combined := descText + "\n" + content
...
// processDualModalBatch processes a batch of nodes with dual-modal embedding.
...
func (p *EmbeddingPipeline) buildQuantizedVectorFile(ctx context.Context, repo string) error {
	...
	// Build vector file
	writer := vecfile.NewVectorFileWriter(dim, params.Scale, params.Offset)
	...
	vecPath := vecfile.VectorFilePath(p.dataDir, repo)
