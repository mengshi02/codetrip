...
func TestRunDualModal_Basic(t *testing.T) {
	...
	result, err := pipeline.RunDualModal(context.Background(), "dualrepo", false)
	if err != nil {
...
func TestRunDualModal_RequiresIndex(t *testing.T) {
	...
	result, err := pipeline.RunDualModal(context.Background(), "emptyrepo", false)
	if err != nil {
...
// TestRunDualModal_Incremental verifies that incremental mode skips
// nodes whose content hash has not changed since the last embedding.
func TestRunDualModal_Incremental(t *testing.T) {
	dir := t.TempDir()
	s, err := store.Open(store.Config{Path: dir})
	if err != nil {
		t.Fatalf("open store: %v", err)
	...
	gs := graph.NewGraphStore(s, "increpo")
	...
	node.Props.SetProp("content", "func doWork() {}")
	if err := gs.AddNode(node); err != nil {
		t.Fatalf("add node: %v", err)
	}
	...
	result1, err := pipeline.RunDualModal(context.Background(), "increpo", true)
	if err != nil {
		t.Fatalf("first RunDualModal: %v", err)
	}
	if result1.NodesEmbedded != 1 {
		t.Errorf("first run: NodesEmbedded = %d, want 1", result1.NodesEmbedded)
	}
	if result1.Skipped != 0 {
		t.Errorf("first run: Skipped = %d, want 0", result1.Skipped)
	}

	// Second run with incremental=true: should skip (content unchanged)
	result2, err := pipeline.RunDualModal(context.Background(), "increpo", true)
	if err != nil {
		t.Fatalf("second RunDualModal: %v", err)
	}
	if result2.Skipped != 1 {
		t.Errorf("second run: Skipped = %d, want 1", result2.Skipped)
	}
	if result2.NodesEmbedded != 0 {
		t.Errorf("second run: NodesEmbedded = %d, want 0 (all skipped)", result2.NodesEmbedded)
	}
}
...
func TestRunDualModal_MultipleNodes(t *testing.T) {
	...
	result, err := pipeline.RunDualModal(context.Background(), "multirepo", false)
	if err != nil {
		t.Fatalf("RunDualModal: %v", err)
	}
...
func TestRunDualModal_NoEmbeddableNodes(t *testing.T) {
	...
	result, err := pipeline.RunDualModal(context.Background(), "norepo", false)
	if err != nil {
		t.Fatalf("RunDualModal: %v", err)
	}
	if result.NodesEmbedded != 1 {
...
func BenchmarkRunDualModal(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		// Use non-incremental to always re-embed
		_, _ = pipeline.RunDualModal(context.Background(), "benchrepo", false)
	}
}